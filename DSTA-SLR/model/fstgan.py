...
class unit_san(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.use_confidence_graph = kwargs.get("use_confidence_graph", False)
        self.confidence_graph_lambda = kwargs.get("confidence_graph_lambda", 1.0)
        ...
        self.Linear_weight = nn.Parameter(
            torch.zeros(in_channels, self.inter_channels * num_subset),
            requires_grad=True,
        )
        nn.init.normal_(
        ...
        self.Linear_bias = nn.Parameter(
            torch.zeros(1, self.inter_channels * num_subset, 1, 1),
            requires_grad=True,
        )
        ...
        if isinstance(A, np.ndarray):
            physical_prior = A.sum(axis=0) if A.ndim == 3 else A
            physical_prior = torch.from_numpy(physical_prior.astype(np.float32))
        elif torch.is_tensor(A):
            physical_prior = A.sum(dim=0) if A.dim() == 3 else A
            physical_prior = physical_prior.float()
        else:
            physical_prior = torch.tensor(A, dtype=torch.float32)
        self.register_buffer("physical_prior", physical_prior.unsqueeze(0).unsqueeze(0))
        self.physical_alpha = nn.Parameter(torch.ones(1, num_subset, 1, 1))
        if self.use_confidence_graph:
            self.confidence_gate = nn.Sequential(
                nn.Conv2d(3, num_subset, kernel_size=1, bias=True), nn.Sigmoid()
            )
    ...
    def forward(self, x0, confidence_map=None):
        ...
        if self.use_confidence_graph and confidence_map is not None:
            confidence_summary = confidence_map.mean(dim=2)
            confidence_i = confidence_summary.unsqueeze(-1).expand(
                -1, -1, -1, self.num_point
            )
            confidence_j = confidence_summary.unsqueeze(-2).expand(
                -1, -1, self.num_point, -1
            )
            pair_features = torch.cat(
                [confidence_i, confidence_j, (confidence_i - confidence_j).abs()],
                dim=1,
            )
            confidence_gate = self.confidence_gate(pair_features)
            attention = (
                self.confidence_graph_lambda * attention * confidence_gate
                + self.physical_alpha * self.physical_prior.expand(
                    attention.size(0), -1, -1, -1
                )
            )
...
class ConfidenceAwareTemporalRectifier(nn.Module):
    ...
    def shift_tensor(self, tensor, offset):
        if offset == 0:
            return tensor
        if offset > 0:
            padded = F.pad(tensor, (0, 0, offset, 0), mode="replicate")
            return padded[:, :, : tensor.size(2), :]
        padded = F.pad(tensor, (0, 0, 0, -offset), mode="replicate")
    ...
    def forward(self, x, confidence_map):
        ...
        for offset in range(-self.radius, self.radius + 1):
            shifted_x = self.shift_tensor(x, offset)
            shifted_conf = self.shift_tensor(confidence_map, offset)
            similarity = F.cosine_similarity(x, shifted_x, dim=1).unsqueeze(1)
            score_terms.append(similarity + self.confidence_scale * shifted_conf)
            shifted_features.append(shifted_x)
        weights = torch.softmax(torch.stack(score_terms, dim=1), dim=1)
        repaired = sum(
            weights[:, idx] * shifted_features[idx]
            for idx in range(len(shifted_features))
        )
        return confidence_map * x + (1.0 - confidence_map) * repaired
...
class Block(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        self.stride = stride
        self.use_temporal_rectification = kwargs.get("use_temporal_rectification", False)
        ...
        self.temporal_rectifier = (
            ConfidenceAwareTemporalRectifier(
                window_size=kwargs.get("temporal_window_size", 5)
            )
            if self.use_temporal_rectification
            else None
        )
    ...
    def forward(self, x, confidence_map, keep_prob):
        y = self.san(x, confidence_map)
        if self.attention:
            # spatial attention
            se = y.mean(-2)  # N C V
        ...
        if self.temporal_rectifier is not None:
            y = self.temporal_rectifier(y, confidence_map)
        ...
        y = self.relu(y + x_skip)
        if confidence_map is not None and self.stride > 1:
            confidence_map = F.avg_pool2d(
                confidence_map,
                kernel_size=(self.stride, 1),
                stride=(self.stride, 1),
            )
        return y, confidence_map
...
class Model(nn.Module):
    def __init__(
        ...
        self.has_confidence_input = has_confidence_input
        self.coord_channels = in_channels - 1 if has_confidence_input else in_channels
        self.use_confidence_encoding = use_confidence_encoding
        self.use_confidence_graph = use_confidence_graph
        self.confidence_graph_lambda = confidence_graph_lambda
        self.use_temporal_rectification = use_temporal_rectification
        self.temporal_window_size = temporal_window_size
        if self.use_confidence_encoding and not self.has_confidence_input:
            raise ValueError(
                "use_confidence_encoding requires has_confidence_input=True"
            )
        self.encoded_in_channels = (
            self.coord_channels * 2 + 1 if self.use_confidence_encoding else in_channels
        )

        if self.use_confidence_encoding:
            self.coord_bn = nn.BatchNorm1d(num_person * self.coord_channels * num_point)
        else:
            ...
            [
                (
                    Block(
                        ...
                        use_confidence_graph=use_confidence_graph,
                        confidence_graph_lambda=confidence_graph_lambda,
                        use_temporal_rectification=use_temporal_rectification,
        ...
        if self.use_confidence_encoding:
            bn_init(self.coord_bn, 1)
        else:
            bn_init(self.data_bn, 1)

    def forward(self, x, keep_prob=0.9, return_features=False):
        N, C, T, V, M = x.size()
        if self.has_confidence_input:
            confidence_map = x[:, self.coord_channels : self.coord_channels + 1]
            confidence_map = torch.nan_to_num(
                confidence_map, nan=0.0, posinf=1.0, neginf=0.0
            ).clamp(0.0, 1.0)
            confidence_map = (
                confidence_map.permute(0, 4, 1, 2, 3)
                .contiguous()
                .view(N * M, 1, T, V)
            )
        else:
            confidence_map = None
        base_confidence_map = confidence_map

        if self.use_confidence_encoding:
            coords = x[:, : self.coord_channels]
            coords = coords.permute(0, 4, 3, 1, 2).contiguous().view(
                N, M * V * self.coord_channels, T
            )
            coords = self.coord_bn(coords)
            coords = (
                coords.view(N, M, V, self.coord_channels, T)
                .permute(0, 1, 3, 4, 2)
                .contiguous()
                .view(N * M, self.coord_channels, T, V)
            )
            x = torch.cat([coords, confidence_map, coords * confidence_map], dim=1)
        else:
        ...
        for u, blk in enumerate(self.layers):
            x, confidence_map = blk(
                x,
                confidence_map
                if (self.use_confidence_graph or self.use_temporal_rectification)
                else None,
                1.0 if u < self.drop_layers else keep_prob,
            )
        ...
        final_confidence_map = (
            confidence_map if confidence_map is not None else base_confidence_map
        )
        ...
        pooled_feature = x.reshape(N, M, c_new, -1).mean(3).mean(1)
