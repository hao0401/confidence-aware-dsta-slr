...
def apply_confidence_mode(
    ...
    if mode == "original":
        return confidence
    if mode == "constant":
        return np.ones_like(confidence, dtype=confidence.dtype) * float(constant_value)
    if mode == "shuffle":
        ...
        return flat[perm].reshape(confidence.shape)
...
def make_bone_stream(data_numpy):
    ...
    bone_data[CONFIDENCE_CHANNEL] = sanitize_confidence(ori_data[CONFIDENCE_CHANNEL])
    for source, target in BONE_PAIRS:
        bone_data[:COORD_CHANNELS, :, target, :] = (
            ori_data[:COORD_CHANNELS, :, target, :]
            - ori_data[:COORD_CHANNELS, :, source, :]
        )
        bone_data[CONFIDENCE_CHANNEL, :, target, :] = (
            ori_data[CONFIDENCE_CHANNEL, :, target, :]
            + ori_data[CONFIDENCE_CHANNEL, :, source, :]
        ) / 2.0
    bone_data[CONFIDENCE_CHANNEL] = sanitize_confidence(
        bone_data[CONFIDENCE_CHANNEL]
    )
    return bone_data


def make_motion_stream(data_numpy):
    ori_data = data_numpy.copy()
    t_size = ori_data.shape[1]
    motion_data = np.zeros_like(ori_data)
    for t in range(t_size - 1):
        motion_data[:COORD_CHANNELS, t, :, :] = (
            ori_data[:COORD_CHANNELS, t + 1, :, :]
            - ori_data[:COORD_CHANNELS, t, :, :]
        )
        motion_data[CONFIDENCE_CHANNEL, t, :, :] = np.minimum(
            ori_data[CONFIDENCE_CHANNEL, t + 1, :, :],
            ori_data[CONFIDENCE_CHANNEL, t, :, :],
        )
    motion_data[CONFIDENCE_CHANNEL] = sanitize_confidence(
        motion_data[CONFIDENCE_CHANNEL]
    )
    return motion_data


def make_stream(data_numpy, bone_stream=False, motion_stream=False):
    data_numpy = np.array(data_numpy, copy=True)
    data_numpy[CONFIDENCE_CHANNEL] = sanitize_confidence(
        data_numpy[CONFIDENCE_CHANNEL]
    )
    if bone_stream:
        data_numpy = make_bone_stream(data_numpy)
    if motion_stream:
        data_numpy = make_motion_stream(data_numpy)
    return data_numpy


def sample_sequence(data_numpy, window_size, random_choose):
    if window_size <= 0:
        return data_numpy
    if random_choose:
        return tools.random_sample_np(data_numpy, window_size)
    return tools.uniform_sample_np(data_numpy, window_size)
...
def compute_sample_quality(
    ...
    data_numpy = np.array(data_numpy, copy=True)
    data_numpy[CONFIDENCE_CHANNEL] = apply_confidence_mode(
        data_numpy[CONFIDENCE_CHANNEL],
    ...
    if window_size > 0:
        data_numpy = sample_sequence(data_numpy, window_size, random_choose=False)
    return float(sanitize_confidence(data_numpy[CONFIDENCE_CHANNEL]).mean())
...
class Feeder(Dataset):
    def __init__(
        ...
        self.missing_joint_prob = missing_joint_prob
        ...
        if self.lap_pe:
            if self.drop_confidence_channel:
                raise ValueError("drop_confidence_channel is not supported with lap_pe")
    ...
    def load_data(self):
        ...
        data_count = int(self.data.shape[0])
        if label_count != data_count:
            aligned_count = min(label_count, data_count)
            print(
                f"[Feeder] Mismatched sample count for {self.data_path} / {self.label_path}: "
                f"data={data_count}, labels={label_count}. Trimming to {aligned_count}."
            )
            self.sample_name = self.sample_name[:aligned_count]
            self.label = self.label[:aligned_count]
            self.data = self.data[:aligned_count]
    ...
    def get_mean_map(self):
        ...
        self.coord_std_map = np.maximum(
            self.std_map[:COORD_CHANNELS],
            np.ones_like(self.std_map[:COORD_CHANNELS]) * 1e-6,
        )
    ...
    def __getitem__(self, index):
        ...
        data_numpy = make_stream(
            data_numpy,
            bone_stream=self.bone_stream,
            motion_stream=self.motion_stream,
        )

        # if self.random_choose:
        #    data_numpy = tools.random_choose(data_numpy, self.window_size)

        data_numpy = sample_sequence(data_numpy, self.window_size, self.random_choose)
        ...
        if self.noise_std > 0:
            noise = np.random.normal(
                ...
                size=data_numpy[:COORD_CHANNELS].shape,
            ).astype(data_numpy.dtype)
            data_numpy[:COORD_CHANNELS] += noise

        if self.missing_joint_prob > 0:
            missing_mask = (
                np.random.rand(*data_numpy[CONFIDENCE_CHANNEL].shape)
                < self.missing_joint_prob
            )
            coord_mask = np.broadcast_to(
                missing_mask[None, ...], data_numpy[:COORD_CHANNELS].shape
            )
            data_numpy[:COORD_CHANNELS][coord_mask] = 0
            data_numpy[CONFIDENCE_CHANNEL][missing_mask] = 0
        ...
        if self.drop_confidence_channel:
            data_numpy = data_numpy[:COORD_CHANNELS]
